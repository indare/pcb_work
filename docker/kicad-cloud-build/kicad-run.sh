...
# Windows の既定コンソールコードページ (cp932) では、ERC 集計が出す日本語で
# クラッシュする。python3 呼び出しは常に UTF-8 に固定する。
export PYTHONUTF8=1
...
# drift サブコマンドは 2026-09-03 に削除した。wire_circuit_design.py を一時ディレクトリで
# 回して実図と突き合わせる仕組みだったが、構成刷新でそのスクリプトはどのシートも
# 書き出さなくなり（現行シートは build_motherboard.py / build_daughter.py が作る）、
# 比較対象が旧構成の凍結シートだけになった。理由と代わりの検証手段は README.md
# 「生成スクリプトと実図の突き合わせ」。突き合わせ本体の sch_drift.py は
# 手で回せるように残してあるが、ワイヤもジャンクションも見ないので接続の検証には使えない。
