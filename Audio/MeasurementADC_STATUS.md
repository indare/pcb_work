...
| 残差 | 約 1175 Hz 系の弱い混入（実用上は許容）。**2026-09-03 の実測では 1205.7 Hz へ 31 Hz ドリフト** |
| スプリアス | **`fs/64` = 750 Hz の倍音列を新たに発見**（BRINGUP「混入スペクトルの実測」）。**`750×4 = 3000 Hz` が 1 kHz 音の H3 と衝突するので試験周波数は 1320 Hz を使う** |
| ビン当たりの床 | L −123.9 / R −117.6 dBFS（無信号、N=16384、Hann） |
| **L/R 入れ替わり** | **信号源の L/R と ADC の L/R が入れ替わっている**（2026-09-03 実測で確定）。`AmpModule` の `J402` は **pin1=`R_IN` / pin2=`L_IN`** という逆順なので、ケーブルを「pin1=左」で繋ぐと入れ替わる。試験音のレベルを片ch だけ 11dB 下げたら **ADC 側は逆のch が 11.2dB 下がった**ので確定 |
| ch 間ゲイン差 | **ADC-R 側の経路が ADC-L 側より約 6dB 高い**。無信号の雑音も信号も一貫して 6dB 高く、dBc では同等なのでゲイン差。**R が先にクリップする**ので試験音は R を下げて作る |
...
| 接続 | **Amp → AdcBuffer（OPA1652 DIP / Q5M411）→ スペアナ**。Controll 2 段は外している。EQ は今号では使わない |
