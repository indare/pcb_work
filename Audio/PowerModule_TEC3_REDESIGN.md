...
TMR 6 の次を SIP 穴に無理に載せると Cout が減る（§1.2）。**PowerModule の再設計が可**なら、電流と Cout を同時に取れるのは 15 W 級の **2″×1″（50.8×25.4）** が本命。

| 候補 | ±15 | Cout | 外形 | メモ |
|---|---|---|---|---|
| **Recom REC15-1215D/H2/M** | **±500 mA** | **±1000 µF** | 2″×1″ / DIP-24 相当 | 電流×2.5・Cout は TMR 6 より厚い。突入 150% 可 |
| **Mean Well DKA15B-15** | ±500 mA | **680 µF** | 50.8×25.4×10.2 金属 | 内蔵 EMI Class A。ヒューズ推奨 |
| Traco THN 20-1223 | ±667 mA | 350 µF | **1″×1″** | いちばん小さい 20 W。Cout は 1 Amp 構成（≈170 µF）なら可。Amp×2 同時バルクだとギリギリ |
| Traco THN 15-1223N | ±500 mA | 250 µF | 1″×1″ | THN 15 無印は NRND。Cout は TMR 9 並で薄い |
| Traco TEL 15-1223N | ±500 mA | 270 µF | DIP-16 薄型 | Cout 薄め |
| Traco TEN 15-1223 | ±500 mA | **165 µF** | 2″×1″ | 電流は十分、Cout が今の 170 µF 概算で **仕様外** |

Amp×10 を 5532 のまま同時通電しても ≈111 mA → 15 W（500 mA）なら定格の 22%。AK05 150 mA 説でも 1 枚なら入る。

ピンは SIP-8 と違う（2″×1″ は業界標準の 2×1 Dual）。二次 LC・F1（入力は 15 W/0.85/9 V ≈ 2 A 級 → **3 A SB 前後**）・FP を取り直す。

**再設計するなら REC15-1215D か DKA15B-15。** 小さくしたいなら THN 20 で島バルクを 100 µF のまま 1 枚通電に限る。
...
5. 基板再設計可なら **REC15-1215D または DKA15B-15**（§1.3）。SIP 穴への TMR 9/12 載せ替えは Cout が減るので使わない（§1.2）。計測と Amp の 2 台分けも可
