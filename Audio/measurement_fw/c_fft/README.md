...
## 実機の測定結果（n=1024, RP2350 @ 48kHz 系）

| 項目 | C `fft_q15` | Python viper |
|------|-------------|--------------|
| ピーク bin | 21 | 21 |
| ピーク dBFS | −6.02 | −6.02 |
| 1 回あたり | **13.20 ms** | 20.54 ms |

−6 dBFS・bin 21 の正弦波で、ピーク位置とレベルは一致。速度は約 1.56 倍。
...
レベル読み（0〜−80 dBFS）が誤差 0.01 dB 級で一致することを確認します。
...
FFT は 1 フレーム 20.5 → 13.2 ms になりますが、以前の実測では描画が
ボトルネックだったため、体感 fps への寄与は限定的と見ています。
