...
print("EXACT", all(out[i] == pw[i] for i in range(len(out))))
gc.collect()
t0 = time.ticks_us()
for _ in range(50): sink = fc.power_into(x, out)
print("C_MS", round(time.ticks_diff(time.ticks_us(), t0) / 50000.0, 3))
gc.collect()
t0 = time.ticks_us()
for _ in range(50): sink = fp.power(x)
print("PY_MS", round(time.ticks_diff(time.ticks_us(), t0) / 50000.0, 3))
