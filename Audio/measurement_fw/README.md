...
- FFT 点数は既定 **1024**（Δf≈47 Hz）。**2k** は低域分解能が良い（Δf≈23 Hz）が fps は落ちる
  - **両立させるならオーバーラップ**（長い窓 + 短い hop）。Δf は窓長 `fs/N`、更新周期は
    ホップ長 `hop/fs` で決まるので、非オーバーラップだと連動してしまう両者を切り離せる。
    **未実装。** 壁は Pico 側の解析予算（[`c_fft/README.md`](c_fft/README.md) の実測で
    `fft_q15` は n=1024・1ch あたり **1.91 ms**）、次が LCD の SPI。
    **低域 IIR の整定（上の `Q/f0`）は hop を短くしても縮まらない別制約。**
  - 狙いは**表示の見え方だけ**で、THD 測定の足しにはならない（上の
    「THD 測定に `spectrum.py` を使わない理由」）。DUT の弁別は
    `capture_raw.py` → PC 側 float64 の経路が担っていて、そちらは 3σ で約 −104 dBc
...
## ⚠ USB CDC でキャプチャを流してはいけない（2026-09-07 実測）

**`sys.stdout` へ base64 を流すと 64 バイト単位で取りこぼす。** 16384 フレームの
キャプチャで**毎回 64〜318 文字が欠落**し、base64 が復号できなかった。
PowerShell / `cmd` / Git Bash のどれでも同じだったので、シェルではなく**転送そのもの**。

**「少しだけ欠けた」では済まない。** 64 バイト落ちると以降のサンプルが 16 ワードずれて
L/R が入れ替わり、波形に段差が入る。段差は広帯域スプラッタになるので −78 dBFS の
スパー測定には使えない。

→ **`capture_to_file.py`**（Pico のファイルに書いて `mpremote cp` で回収）を使う。
`cp` は raw REPL の確認応答付きプロトコルなので取りこぼさない。**欠落 0 で通る。**

`capture_raw.py` は残してあるが、**そのままでは実用にならない**。

## ⚠ 起動直後に捕捉へ入ると USB が列挙されない（2026-09-07 実測）

`main.py` が起動直後に `rx.wait()` でブロックすると、**USB の列挙が完了しない**。
症状は「電源投入後に COM ポートが一切現れない」。ケーブルもポートも正常で、
**BOOTSEL では `RPI-RP2` として見えるのに、通常起動では何も名乗らない。**

**ソフトリセット（`mpremote reset`）では USB の列挙が維持されるので、この罠は
完全な電源断のときだけ出る。** 切り分けの本番はまさにそれなので必ず踏む。

→ `main_flash.py` は冒頭に **`time.sleep(5)`** を置いている。これで完全電源断からでも
列挙された。**捕捉を起動時に走らせる `main.py` を書くときは必ず猶予を入れること。**

### 詰んだときの戻し方

**BOOTSEL を押しながら USB を挿す** → `F:` に `RPI-RP2` が現れる →
**`../../tmp_fw/RPI_PICO2-fft_set_n.uf2` をコピー**。2026-09-07 に実証済みで、
**ファイルシステムは無傷のまま復旧した**。

**⚠ `tmp_fw/RPI_PICO2-latest.uf2` は UF2 ではない。** 中身は nginx の **404 HTML**（162 バイト）で、
ダウンロード失敗が残っている。焼くなら `RPI_PICO2-fft_set_n.uf2`（`spectrum.py` が
`fft_q15.set_n` を使うので、この C モジュール入りカスタム版が正）。

## ⚠ USB を挿したまま基板の電源を切っても Pico は落ちない

Pico の `VBUS`（USB の 5V）が `VSYS` に入っているので、基板の `+5V_D`（`D701` 経由）が
落ちても**USB 側から給電され続ける**。**Pico をリセットしたいなら
「基板の電源を切る → USB を抜く → USB を挿す → 基板の電源を入れる」の順**が要る。

逆に言えば、**USB を抜いても Pico は基板給電で動き続ける**。A1 の切り分けはこれを利用している。
