...
**次号 PCB で置き方を決め打ちしない。いまの ver（1 台）で効果を測ってから設計確認する。**

| 分担 | いつ効くか |
|---|---|
| 解析 / 表示 | 解析は予算内なのに LCD 間引きが常連 → GIL・SPI・RAM 分離の価値 |
| L / R | hop を詰めても 1 台の解析自体が足りない → スループット向き（重い） |

機械配置（縦スタック／横並び）は効果次第。積むなら **全ピン並列は不可**で、必要なピンだけ伸ばす:

- 共有: 電源・GND・UART など数本
- 下（解析想定）: GP0–2 I2S、必要なら GP15。GP9 は触らない
- 上（表示想定）: GP5–8 / GP18–19 LCD、GP10–13 タッチ

v0.1 で見る判断材料:

1. 連続 DMA＋hop で `(N, hop)` が core1 実時間に収まるか
2. 解析 OK・描画だけ間引き常連か → 表示分離の価値
3. hop=512 でも解析不足か → 1 台の C 化深掘り or L/R 分割
...
Pico2台目や縦/横配置は次号で決め打ちせず、v0.1の効果次第。
積むなら必要ピンだけ（電源・GND・UART＋役割ごとのGPIO独占）。
```
