...
⚠⚠ **この方式は「Pico の電源を落とさない」ときしか使えない**（2026-09-07 実測）。
   **USB 未接続で起動した Pico は、後から挿しても USB CDC が列挙されない。**
   LED は点いていて Pico 自体は動いているのに、ホストからは一切見えない。
   通信を戻すにはリセットが要り、リセットすれば RAM の保持は消える。
   → **基板ごと電源を入れ直す形で採るなら `capture_to_flash.py` を使うこと。**
   こちらはラッチした時点でフラッシュに書くので、リセットしても残る。

⚠ **読み出しの `dump()` は `sys.stdout` へ流すので取りこぼす**（README「USB CDC で
   キャプチャを流してはいけない」）。ファイル経由で読むなら `dump_hold_to_file.py`。
