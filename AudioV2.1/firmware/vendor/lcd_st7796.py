"""ST7796S（Waveshare 29318）— AudioV2 用。

`Audio/measurement_fw/lcd.py` から分岐。ピンの正は MeasureControl の Pico 注記。
⚠ AudioV2 では GP16 = ENC_INTA なので、旧計測盤の SPI MISO=GP16 は使わない。
"""
import framebuf
import time
...
class Lcd:
    def __init__(self):
        ...
        Pin(3, Pin.IN)
        Pin(4, Pin.IN)
        # MISO はパネル書き込みに未使用。GP16 は ENC_INTA なので空けたピンをダミーに。
        self.spi = SPI(
            0,
            baudrate=40_000_000,
            polarity=0,
