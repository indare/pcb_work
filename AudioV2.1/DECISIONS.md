...
- 2026-09-26: ch の LDO の出力の容量を実効で約 5 µF に（§3-7）。逆流のダイオードは ch・トーンとも置き場だけ。空のソケットの ch は選ばない（§6-2）
...
- **状態**:
  ...
  - いったんの決め（ユーザー判断 2026-09-26。材料は [review/njw1194_io.md](review/njw1194_io.md)・[_review](review/njw1194_io_review.md)）:
    ...
    6. **トーンの ±7 V の LDO に、出力から入力へのショットキーの置き場**（実装はしない。置くなら 1 A 級のショットキーが載る大きさ。ch の LDO と同じ扱い、§3-7）。理由: TPS7A49／TPS7A30 は出力が入力を 0.3 V 越えてはいけない（絶対最大、DS p5 の画像で確認）。電源断中に源が鳴っていると入力のクランプからレールが押し上げられ（直列 1 kΩ でも約 ±0.6〜0.8 V〔推論〕）、源が無くても ±15 V が ±7 V より先に落ちれば差が開きうる〔推論〕。ブリーダは通常時に 1 レールで約 186 mW を食い成り立たない
...
### 3-7 ch LDO: CNR/SS は必須、出力の容量は実効で約 5 µF に抑える（2026-09-26 に別の理由で上限を戻した）
...
- **状態**:
  ...
  - いったんの決め（ユーザー判断 2026-09-26。材料は [review/ch_ldo_reverse.md](review/ch_ldo_reverse.md)・[_review](review/ch_ldo_reverse_review.md)、スクリプトは `spice/ch_ldo_reverse/`）:
    1. **出力の容量は実効値で約 5 µF に抑える**（DS の安定の最小 2.2 µF の内側。公称 10 µF のセラミックは 12 V の直流で実効値が下がるので、実効値で選ぶ）。理由: 電源断のとき LDO の出力が入力を越える（絶対最大は OUT–IN・FB–IN・NR/SS–IN とも +0.3 V、TPS7A30 は −0.3 V〔両 DS p5、画像で確認〕）。越え方は出力の容量と石の電流で決まり、手持ちで電流の少ない石（OPA2140 約 3.6 mA、OPA1652 約 4 mA）だと 10 µF でも主電源断で +2.8〜5.4 V、ふつうの切替で +1.9 V 開く〔シミュレーション〕。出力の素子は TPS7A49 が PNP・TPS7A30 が NPN〔DS のブロック図〕で、数 V 開くと逆向きの接合に掛かる
    2. **出力→入力のダイオードと GND クランプは実装しない**。置き場は作り、置くなら 1 A 級のショットキーが載る大きさにする（BAT54 では文字どおりには守れない。NR/SS ピンはどのみち切替のたびに 0.3 V の制限を越える）。片側のレールだけ失う筋で負側の出力が大きく押し上げられるのは、短絡級の故障のときだけ（ヒューズ・接点の開放、0.45 V/ms の落ち方では +0.22 V）
    3. **帰還の分圧は合計 100 kΩ 級**。理由: FB ピンの電流（100 nA max）による出力電圧の誤差（1 MΩ 級で 0.75 %、100 kΩ 級で 0.08 %）
    4. 運用の前提: **選ぶ ch には必ず石が挿さっている**（空のソケットの ch は選ばない、§6-2）。空のソケットを選ぶと出力の容量が帰還の分圧でしか抜けず、+11.5〜12 V が 1〜10 s 残る
    5. 実物で 1 回確かめる: OPA2140 を挿し、主電源を切るときとふつうの切替のときに LDO の OUT と IN を見る
...
- **運用（ソケット）**: 選ぶ ch には必ず石を挿しておく。空のソケットの ch は選ばない（§3-7。ファームは空のソケットを検出できないので運用の約束）。状態: いったんの決め（ユーザー判断 2026-09-26）
