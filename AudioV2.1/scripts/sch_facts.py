...
102 µF ではなく約 250 µF）は、シートが何回インスタンス化されているかを数え直しただけで出た。
    ...
    python3 AudioV2.1/scripts/sch_facts.py all           # 査読の入口。まずこれ
    python3 AudioV2.1/scripts/sch_facts.py tree          # 階層とインスタンス数（×何枚）
    python3 AudioV2.1/scripts/sch_facts.py bom           # 部品表（インスタンス展開後）
    python3 AudioV2.1/scripts/sch_facts.py rails         # レールごとの容量・ピン数の積み上げ
    python3 AudioV2.1/scripts/sch_facts.py nets --net +15V
    python3 AudioV2.1/scripts/sch_facts.py iface         # 親のシートピン vs 子の階層ラベル
    python3 AudioV2.1/scripts/sch_facts.py dangling      # どこにも載っていないピン
    python3 AudioV2.1/scripts/sch_facts.py aliases       # 1本のネットに別名が複数
    python3 AudioV2.1/scripts/sch_facts.py pin <REF>     # 部品のピンごとのネット名

`--json` で機械可読。`--root` で別の階層ルートを指定できる（既定 AudioV2.1/AudioV2Case.kicad_sch）。
...
def main() -> int:
    ...
    ap.add_argument("--root", default="AudioV2.1/AudioV2Case.kicad_sch")
