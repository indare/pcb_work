...
帰還・出力段）は座標も配線もそのまま。触るのは入力部・切替部・階層ピンだけ。
    ...
    python3 AudioV2.1/scripts/build_ampchannel.py --dry-run
    python3 AudioV2.1/scripts/build_ampchannel.py

⚠ **これは一度きりの移行スクリプトで、冪等ではない。** 変換済みのAmpChannel を
もう一度食わせると「外す部品が見つからない」で **書き込む前に**落ちる（そう作ってある）。
やり直すなら `git checkout AudioV2.1/AmpChannel.kicad_sch` してから回すこと。
"""
...
def main() -> int:
    ...
    if out:
        ...
        print(f"書き換え: AudioV2.1/AmpChannel.kicad_sch ({len(out)} bytes)")
    return 0
