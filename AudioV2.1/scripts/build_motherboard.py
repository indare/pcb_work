...
として読むので、手描きの配線とジャンクションがそのままルートへ移る。
    ...
    python3 AudioV2.1/scripts/build_motherboard.py           # 書き出す
    python3 AudioV2.1/scripts/build_motherboard.py --dry-run # 内訳だけ
...
# --- 娘基板スロット（D18 のピン割当）------------------------------------
#
# 両版で完全に同一のヘッダ。2x8 のうち、
# 1..10 = 電源 / I²C、11..16 = N.C.（旧 ADDR・SEL。番地は娘ジャンパ、SEL は娘 MCP）。
...
# 旧スロット番地・SEL（廃止）。ピン 11..16 は母板・娘とも N.C.。
SLOT_ADDR = {}
SLOT_SEL_NETS = {}
            ...
            for lib, ref, value, at, nets, fp, desc in (
                ...
                for num, net in nets.items():
                    ...
                    els.append(sch_import.Element(
                        ...
                        None, net, (x, y)))
                # J_PWR pin 11..16 = N.C.（旧 ADDR / SEL）
                if ref.startswith("J_PWR"):
                    for num in range(11, 17):
                        x, y = tips[str(num)]
                        els.append(sch_import.Element(
                            "no_connect", _no_connect(x, y), None, None, (x, y)))

        # スイッチ版 SEL は娘 MCP に移した（親 U_IO101 廃止）。
        # 番地も娘ジャンパ（6 段）。ここには MCP を置かない。
...
def main() -> int:
    ...
    if out:
        ...
        print(f"書き出し: AudioV2.1/AudioV2Case.kicad_sch ({len(out)} bytes)")
    ...
    if mother.exists():
        mother.unlink()
        print("削除: AudioV2.1/MotherBoard.kicad_sch（中間階層を廃止）")
    return 0
