...
分割は別ステップ。いまは「10ch 1枚」のまま D22 を成立させて検証できる状態にする。
    ...
    python3 AudioV2.1/scripts/build_ampbank.py --dry-run

⚠ **これは一度きりの移行スクリプトで、冪等ではない。** 変換済みのAmpBank を
もう一度食わせると「外す部品が見つからない」で **書き込む前に**落ちる（そう作ってある）。
やり直すなら `git checkout AudioV2.1/AmpBank.kicad_sch` してから回すこと。
"""
...
def main() -> int:
    ...
    if out:
        ...
        print(f"書き換え: AudioV2.1/AmpBank.kicad_sch ({len(out)} bytes)")
    return 0
