...
使い方:
  python3 AudioV2.1/spice/extract_dut_thd.py            # 抽出して JSON へ保存
  python3 AudioV2.1/spice/extract_dut_thd.py --report   # 保存済み JSON から読み値を出す
"""
...
def main() -> None:
    ...
    print("     負荷 DS 2k / 実回路 50k（軽いぶん実 DUT はもっと良い＝差は縮む方向）。")
    print("  ★ よって『DUT より N dB 下』を設計保証値にしてはいけない。")
    print("     設計上限 7Vrms(9.90Vpk) の根拠はスイッチ単独で完結させる:")
    print("     膝(±11V=7.78Vrms)の明確に内側で、その領域の switch 歪みは約 -167dB 級。")
    print("     この表の交差点は物理モデルの検証用（膝と独立に一致した）であって、")
    print("     設計上限の根拠ではない。")
