...
**トーンは NJW1194 に決定**（2026-09-26、DECISIONS §1-6。DIRECT はチップの中で作り、リレーは置かない。PT2314E は却下）。ユーザーが実物を数個押さえる。同居、±7 V は ±15 V から LDO（コモン A_GND）、3 線は UI の MCP から（5 V バッファの置き場）、入力に直列抵抗の置き場、電源投入時に状態をリセット（§1-6）。DIRECT は NJW1194 のスルー（音量を MUTE にしてから切り替える）。未決: 電源の無いチップの入力に源が来ることを受けるか。NJW1194 の前提で決め直す項目は §1-6。実物が来たら測る: 本物か、スルーの切替の段、雑音、電源断のときの入力電流
