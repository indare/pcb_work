...
**トーンは NJW1194 に決定**（2026-09-26、DECISIONS §1-6。DIRECT はチップの中で作り、リレーは置かない。PT2314E は却下）。ユーザーが実物を数個押さえる。同居、±7 V は ±15 V から LDO（コモン A_GND）、3 線は UI の MCP から（5 V バッファの置き場）、入力に直列 1 kΩ（既定）と端子側 1 MΩ、入口と出口に 4.7 µF のフィルム、±7 V の LDO に逆流のショットキーの置き場（実装しない）、電源投入時に状態をリセット（§1-6）。ch の ±12 V の LDO は出力の容量を実効で約 5 µF に抑え、逆流のダイオードは置き場だけ（§3-7）。DIRECT は NJW1194 のスルー（音量を MUTE にしてから切り替える）。未決: 電源の無いチップの入力に源が来ることを受けるか。NJW1194 の前提で決め直す項目は §1-6。実物が来たら測る: 本物か、スルーの切替の段、雑音、電源断のときの入力電流
**残る未決**: -20（縦積みの DIP の手の届き、模型）、後回しの -06・-12・-15・-18・-19・-21。B 項目は実装しながら、C 項目は先に測る（統合リスト §3）。
**電源ルートは R1 に決定**（§3-13。RS6 は設計点の最悪で 63 %、充電器の C1〜C4 に 12 V/3 A）。
**次にやること（2026-09-26、ユーザーと合意）**: 実装の前に、決定の全体を**複数の観点で否定側査読**する（観点の案: ① 電源と故障の形、② 制御・ファーム・手順、③ 音声の経路の品質、④ DECISIONS の食い違い・古い記述・参照切れ、⑤ 部品の入手・熱・実装のしやすさ）。前回の全体評価（[review/design_eval_review.md](review/design_eval_review.md)、09-25）以降に、トーン（NJW1194）・A1（制御 MCP）・B7（毎回全リセット）・電源ルート・ヒューズ・LDO の容量などが変わった。その後、娘: 電源用・音声用リレー、ch ごとの電源ゲート一式と入力の切り替え、デコーダ、B1/B2、排他ゲート。親: RS6、ADC の電源とグランドの付け替え、`TONE` のプルダウン、L7805C、`PD_12V_SW` の検知、制御専用の MCP23017 と電源検知の AND、トーン NJW1194 と ±7 V の LDO）。
