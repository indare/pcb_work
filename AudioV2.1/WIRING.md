...
> **⚠ この文書の本文は旧構成（4枚）の記録で、現行の設計ではない。**
> 2026-09-03 に構成刷新は**方針決定だけでなく実装まで完了した**。現行は
> **母板 ＋ 娘基板2版（スイッチ版 / リレー版、5ch ずつ）＋ 計測制御** の4シートで、
> `ControlPanel` / `PowerModule` / `OutputStage` / `AmpBank` は `AudioV2/legacy/` に凍結済み。
