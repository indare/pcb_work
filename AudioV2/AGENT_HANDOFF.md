...
| **B4'** | **Pico をどちらに寄せるか決める** ← **いまここ** | 残る ERC 49 件のうち **32 件が `/ControlPanel/`**（`U401` Pico と `U402` PT2314 の未使用ピン）。これを決めないと `ControlPanel` の解体（母板へ `PT2314`、計測基板へ UI）に進めない |
| B2 | `PT2314` を母板へ移設 | `DECISIONS.md` で可否が未決。B4' と同時に決まる |
