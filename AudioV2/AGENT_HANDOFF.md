...
**§8 のヒューズ表は数字が合っていない。** 定格を**出力側の電流**から出しているのに
PPTC は**入力側**に置いてあるので、F1602 は 3.4 倍・F202 は 4.4 倍ゆるい。
F1601(1A) と F1602 のトリップ(約1A)が並んでいて**選択性も無い**。作り直しが要る。
...
| D-g | **`U1604`（`MBC2596-01`、計測系の降圧）にヒューズを入れるか** | **2026-09-04 に場所を特定**（走行線の上へ迂回、`F1603` を `(265.43, 34.29)` r90）。**定格は `U1604` の負荷電流待ち＝実測が要る。** 電源ツリー全体は `DECISIONS.md`「計測/制御基板の電源ツリー」。未判断。`PD_12V_SW` 直付けで上流は `F1601` のみ。「各コンバータの手前に入れる」（`DECISIONS.md` §8）を徹底するならここにも要る。`MeasureControl` は手編集所有なので KiCad で直接 |
