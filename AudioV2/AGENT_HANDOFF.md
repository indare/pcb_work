...
2. **【新規・最優先級】PCM1804 の測定能力を実測で確定する（issue #33 の到達点）**
   ...
   **当初の問い「どこまでの THD 差を再現性よく比較できるか」に直接答えるのは (a)。**
   ...
   | 石 | THD+N | 前段との差 | データシートの条件 |
   |---|---:|---:|---|
   | NE5532 | −94.0 | — | **TI DS に規定なし（通説値）** |
   | MUSES02 | −100.0 | −6.0 | `AV=+10, 5Vrms, RL=2k` |
   | OPA2604 | −110.5 | −10.5 | `G=1, 3.5Vrms, RL=1k` |
   | OPA1652 | −126.0 | −15.5 | `G=1, 3Vrms` |
   | OPA2140 | −126.0 | 0.0 | `G=+1, 3.5Vrms` |
   | OPA627 | −130.5 | −4.5 | `G=1, 3.5Vrms` |
   | LME49860 | −130.5 | 0.0 | `AV=1, 3Vrms, RL=2k` |
   | OPA1656 | −131.0 | −0.5 | `G=1, 3.5Vrms, RL=600, BW80k` |
   | OPA1612 | −136.0 | −5.0 | `G=+1, 3Vrms` |

   **全体 42 dB / 9石。**（条件は石ごとに違う。#33 の教訓どおり絶対値の直接比較はしない）

   1. **再現性の床（最重要の control）** — **同じ石を抜き差しして**繰り返し測る。
      ソケット接触・熱状態が変わるので、**これが本当の床**。石を替えた差がこれ以下なら見えない
   2. **梯子を降りる** — `NE5532` vs `OPA1612`（42 dB 差）は自明に見えるはず。
      そこから差の小さい対（`LME49860` vs `OPA1656` の 0.5 dB が最難）へ降り、
      **差が床に埋もれたところが分解能**
   3. H2/H3 は**複素振幅**（振幅と位相）で記録する。位相がないと差分が取れない
