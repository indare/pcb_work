...
>
> **総量の段階で落とせるもの:** ②より**上**に出る `DG507B`(−70.3) と
> `ADG1407` @9.2Vrms(−96.1) は、②が多少動いても順序が変わらない大きさ。**ここは確定。**
> それ以外は総量では判断できず、次数別の実測待ち。
> `TMUX7612` は 4〜5 Vrms なら 62 dB 差で、次数別に見ても問題になりようがない
> ＝ **膝より下で使うのが前提**。
>
> **「相対比較は成立」も言い過ぎ。** 正しくは
...
4. **切替素子の選択（リレー版 / アナログスイッチ版）** — §11.1a。**両案を並行検討する方針。
   ...
   歪みは**通常経路（4〜5 Vrms）ではどちらも十分下なので決め手になりにくい**（§1 の⚠）。
   ②より上に出るのは `DG507B` と `ADG1407` @9.2Vrms だけ。
   ただし **`TMUX7612` @9.2Vrms は ②との差が総量で 13 dB しかない**ので「無条件 OK」ではない
   （実際に何dB動くかは次数別の実測待ち）。4〜5 Vrms なら 62 dB 差で問題なし。
   判断は面積・ドライバ・実装容易性・調達で決める。
   判断軸は「面積とドライバ複雑さ」対「実装容易性と調達の堅さ」:
