...
>
> **「床より下だから見えない」も誤り。** 同じ次数の成分は複素ベクトル和で残る。
> 同相最悪の誤差は `20log10(1 + 10^(−margin/20))`:
>
> | マージン | 誤差 | |
> |---:|---:|---|
> | 13 dB | **+1.75 dB** | **`TMUX7612` @9.2Vrms がここ。無視できるとは限らない** |
> | 34 dB | +0.17 dB | `OPA1612`(−136dB) が ADC に乗る場合 |
> | 64 dB | +0.005 dB | `TMUX7612` @4〜5Vrms。事実上ゼロ |
>
> - ②より**上**に出るのは `DG507B`(−70.3) と `ADG1407` @9.2Vrms(−96.1) だけ
> - `TMUX7612` は 4〜5 Vrms なら 64 dB マージンで問題なし ＝ **膝より下で使うのが前提**
> - **「相対比較は成立」も言い過ぎ。** 正しくは
...
4. **切替素子の選択（リレー版 / アナログスイッチ版）** — §11.1a。**両案を並行検討する方針。
   ...
   ただし **`TMUX7612` @9.2Vrms は ②とのマージンが 13 dB しかなく、同相最悪で +1.75 dB
   動きうる**ので「無条件 OK」ではない。4〜5 Vrms なら 64 dB マージンで問題なし。
...
- **⚠ 測定床は3回間違えている。§1 の⚠と spice/README.md「判定の基準線」を必ず読む**:
  ...
  **「THD+N から SNR を引いて高調波 -102.6dB」（THD+N は無加重・SNR は A特性で引き算できない）**。
  さらに **「床より下だから見えない」も誤り** —— 同じ次数は複素ベクトル和で残り、
  マージン 13dB なら同相最悪で +1.75dB 動く（TMUX7612@9.2Vrms がこれ）。
