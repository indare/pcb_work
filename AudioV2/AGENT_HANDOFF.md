...
- アナログスイッチの型番（信号振幅 ±10 V / Ron と平坦度 / ロジックしきい値 / break-before-make / 入手性で選定中）
- ±12 V のままか **±15 V に戻すか**。[DECISIONS.md](DECISIONS.md) は ±12 V を「DKMW20F-12 を使う」というモジュール選定として確定しており、±15 V を否定した記録は無い。同シリーズに `DKMW20F-15`（±15 V / ±660 mA）があり、諦めた 2〜3 dB のヘッドルームが戻る
- 新シート名（仮 `AmpSelector`）
