...
| 状態 | 意味 | 該当シート |
|---|---|---|
| **生成コード所有** | `wire_circuit_design.py <target>` を回せばそのまま正 | **なし**（2026-09-03 に全シートが卒業） |
| **手編集所有** | KiCad上の手編集が正。生成コードはロジック（ネット/ピン対応）のドキュメントとして残すが、機械的に上書きしてはいけない | **Power**（2026-09-01〜）, **Output**（2026-09-01〜）, **AmpBank**（2026-09-02〜）, **AmpChannel**（2026-09-02〜）, **Control**（2026-09-03〜）, **親**（2026-09-03〜） |
| **対象外** | 生成スクリプトが元から出力しないシート。`drift` も比較しない | **MeasureControl**（2026-09-03 に v1 からコピー） |

**2026-09-03 更新 — Control と親が卒業し、生成コード所有は空になった。**
ユーザーが KiCad で親に `MeasureControl` シートを配置し、`ControlPanel` に `-15V`(input) と
`+5V`(output) の階層ピンを追加した（併せて後述の VSYS 給電を削除）。生成スクリプトは
`MeasureControl` を出力しないので、**`parent` を回すとシート配置ごと消える**。`control`/`parent` を
`main()` の `HAND_EDITED` に移し、`--force-control` / `--force-parent` を脱出ハッチにした。

結果として `wire_circuit_design.py` は**どのシートも書き換えないスクリプト**になった。
役割はロジックのドキュメントと `drift` の比較基準だけ。回路の変更は KiCad 側で行う。
