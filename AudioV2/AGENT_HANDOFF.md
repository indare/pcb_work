...
### 次にやること（2026-09-02 夕方 時点）

> **⚠ 作業を再開する前に [issue #33](https://github.com/indare/pcb_work/issues/33) の
> コメント2（ユーザーのレビュー）を読むこと。** そこに未回答の要求が5点ある。

**最大の未決事項は「入力ブロードキャストのトポロジ（A/B/C案）」**。
切替素子（リレー / アナログスイッチ）はその次で、**両案を並行検討する方針**に変わった
（[`DECISIONS.md` §11.1a](DECISIONS.md)）。手順は §5 の優先候補 1・2。

**切替素子を「リレーに確定」から「両案検討」へ戻した理由**（issue #33、同日夕方）:
...
- **測定床は PCM1804 の DR ≈ 112 dB。** `TMUX7612` は 9.2 Vrms でも −115.2 dB で既に床以下。
  −140 dB 以下を追う価値は音質ではなく **IC数・面積・価格**に移る
...
1. **【最優先】入力ブロードキャストのトポロジを決める（A / B / C 案）** — [issue #33](https://github.com/indare/pcb_work/issues/33) コメント2 が
   ...
   - **C案（未実装。#33 コメント2 の要求）**: PT2314 → 共通バッファ → 各DUT個別 coupling C + `Rb 10〜22k` → DUT。
     DC 独立性・小さい source/bias 抵抗・PT2314 負荷の分離を同時に取れる
   - A/B/C で「Ib による DUT 出力の DC 動作点」「カップリングC のリーク感度」「入力雑音」「部品数」を比較する
   - 道具は [`spice/input_topology.py`](spice/input_topology.py)。**雑音の懸念は解決済み**
     （PT2314 の出力抵抗 typ 1.9Ω が支配的で、`Rb` が 1k でも 220k でもノード雑音は 0.18 nV/√Hz）

2. **[issue #33](https://github.com/indare/pcb_work/issues/33) コメント2 の5点に答える（未対応）**
   1. `MAX14753`/`MAX14778` の THD を**「モデル値」と強く明示**（実 Ron(V) 由来ではない。
      特に `MAX14778` は Ron(V) グラフ自体が存在しないので −165dB を選定根拠にしない）
   2. **PCM1804 の測定床（DR ≈ 112 dB）を選定基準に入れる** → −140dB 以下を追う価値は
      音質・測定性能ではなく IC数・面積・価格に限られる
   3. `switch_offiso.py` の非選択DUT歪みに **coherent worst-case** を追加（RSS だけでは
      9ch 完全同相のとき最大 9.5 dB 悪化する）
   4. 入力トポロジに **C案**を追加（上記1）
   5. `MAX14778` の charge-pump spur を「blocker ではない」→ **「一次見積では問題なさそう、実測必須」**へ

3. **切替素子の選択（リレー版 / アナログスイッチ版）** — §11.1a。**両案を並行検討する方針。
   基板として両方を設計してもよい**（ユーザー判断、2026-09-02）。
   歪みはもはや決め手にならない（実運用 4〜5 Vrms ではどちらも測定床の遥か下）。
...
■ 最初にやること
**issue #33 のコメント2（ユーザーのレビュー）を読む。** 未回答の要求が5点あり、
そこに「次は selector より input broadcast topology が主要な未決事項」と書かれている。
  gh issue view 33 --json comments -q '.comments[1].body'
...
■ 未決事項（§5 の優先候補1〜3）
1. **入力ブロードキャストのトポロジ A / B / C 案**（最優先）
   A=各DUT個別 Rb=100k / B=共通 Rb=10k+個別 Rs / **C=共通バッファ+個別 Rb 10〜22k（未実装）**
   A は +入力の DC 抵抗が 1k→100k と100倍になり、DUT を現行と違う条件で比較することになる。
   B は全10石の Ib が合算され1個抜くと全ch が動く。C はその両方を避ける案。
   道具: spice/input_topology.py。**雑音の懸念は解決済み**（PT2314 の出力抵抗 1.9Ω が支配的）
2. **issue #33 コメント2 の5点**（モデル値の明示 / PCM1804 測定床 / coherent worst-case /
   C案の追加 / MAX14778 spur を「実測必須」へ）
...
- **測定床は PCM1804 の DR≈112dB。** TMUX7612 は 9.2Vrms でも -115.2dB で既に床以下。
  -140dB 以下を追う価値は音質でなく IC数・面積・価格
