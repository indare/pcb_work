...
### 2.10 PowerModule 絶縁型DC-DCコンバータ — **`REC10K-2415DAW/H2` に確定**（2026-09-02、回路図未反映）
...
1. **⚠ 最初に `pin 3 = CTRL` の論理をデータシートで確認する。** 現行 `DKMW20` は
   `R.C.` オープン = ON で、ネットリスト上も `unconnected-(U201-R.C.-Pad6)`。
   `REC10K` が同じ極性とは限らない。**逆論理なら固定配線が要る**（常時給電なので ON 側に固定できればよい）
2. シンボル作成（6ピン。KiCad 標準 lib に無ければ `AudioV2.kicad_sym` へ）
3. フットプリント作成（1″×1″ 6-DIP。列間 20.32 mm・2.54 グリッド・穴 Ø1.0）
4. `PowerModule.kicad_sch`（**手編集所有**）で `DKMW20F-15` を差し替え。KiCad 側で直接行う
5. `power_module_wired()` を追随（ドキュメント用途。ここから書き戻さない）
6. `PARTS.md` の該当記述と BOM 再生成
7. 検証: `check_sexpr` / `kicad-run.sh erc` / `netlist` / `drift` / `gen_parts_bom.py --check`
