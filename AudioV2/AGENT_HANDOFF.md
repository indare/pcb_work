...
### ⚠⚠ 大前提: ノイズ対策が先。回路の追い込みはその後（2026-09-03、ユーザー判断）

**2026-09-03 の実機測定で、いま系を縛っているのは回路の精緻化ではなく雑音・混入だと確定した。**

| | 値 | 出どころ |
|---|---:|---|
| DUT（`OPA2134`）の DS 値 | −122 dB | データシート |
| **実測のベースライン H3** | **−76 dBc** | **DUT ではなく系（G8/AdcBuffer/ADC）が作っている** |
| 分解能を縛るもの | **ソケット再現性 −112 dBc** | 抜き差し対照 |
| リレー vs `TMUX7612` の差 | −167 dBc 以下 | 机上 |

**測定系が DUT より 46 dB 悪い状態で、さらにその 60 dB 下の切替素子の差を議論していた。**
切替素子・入力トポロジをこれ以上追い込んでも、**測って確かめる手段が無い**。

#### 実測で判明している雑音・混入（すべて `MeasurementADC_BRINGUP.md` に詳細）

| # | 対象 | 実測 | 性質 |
|---|---|---:|---|
| 1 | **ベースライン H3** | **−76 dBc** | 発生源未特定。G8 / AdcBuffer / ADC のどれか |
| 2 | `fs/64` = 750 Hz 系列 | −80.4 dBFS | **クロック起因**（BCK=64×LRCK）。`750×4=3000Hz` が 1kHz 音の H3 と衝突 |
| 3 | 1205〜1212 Hz 自走源 | −77.8 dBFS | 非同期でドリフト。電源か VCOM 経由 |
| 4 | **ソケット再現性** | **−112 dBc** | **分解能の律速**。接点清掃・ZIF 化が直接効く |
| 5 | ADC-R 経路 | L より 6 dB 高雑音 | `J703`（±15V 入口）に近い基板北側の配置と整合 |
| 6 | デカップリング | `REFINE` §4.3 | ±15V に 0.1µF 各1個、島内バルク無し、`V+` 引き回し 35mm |

#### 順序

1. **雑音対策**（上表 1〜6）。とくに **①ベースラインの発生源特定**が最優先。
   `AmpModule` をバイパスして G8 → `AdcBuffer` 直結で測れば切り分けられる
2. そこで分解能が上がってから、切替素子・入力トポロジの検証へ戻る
...
0. **【最優先】ノイズ対策**（§1 冒頭の⚠⚠を読むこと）。
   **実測でベースライン H3 = −76 dBc、分解能の律速はソケット再現性 −112 dBc。**
   DUT（`OPA2134` の DS 値 −122 dB）より系の方が 46 dB 悪い。
   **ここを改善しないと、以下 1〜4 の差は測って確かめられない。**
   最優先は**ベースラインの発生源特定**（`AmpModule` バイパスで G8 → `AdcBuffer` 直結）。
