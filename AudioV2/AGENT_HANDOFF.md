...
2. **【新規・最優先級】PCM1804 の測定能力を実測で確定する（issue #33 の到達点）**
   ...
   **→ 生サンプルを USB で吸い上げ、PC 側 numpy（float64）で FFT する。**
   ...
   #### 既知の妨害: 1174.8 Hz の倍音列（未解決、L −84.4 / R −70.0 dBFS）

   自走のアナログ源（電源か VCOM 経由）で、こちらのクロックに同期せず
   1172.2 → 1174.8 Hz と動く。**−70dBFS は、見たい −100dB 級より 30dB も上。**

   ただし**周波数で分離できる**。10 バンド表示では帯域内が合算されるので潰れるが、
   **細かい FFT なら別ビンに座る**。試験周波数を選べば H2/H3 と重ならない
   （例: 基本波 1kHz なら H2=2k / H3=3k、妨害は 1175/2350/3525 Hz）。
   **試験周波数は妨害の倍音列と重ならないように選ぶこと。**
   相互変調で側波帯が出ていないかは実測で確認する。
