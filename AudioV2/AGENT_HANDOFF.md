...
> | # | 床 | 値 | FFT長で下がるか |
> |---|---|---:|---|
> | ① | ランダム雑音の bin 床 | −137.3 dBFS | **下がる** |
> | ② | **PCM1804 の歪み** | **THD+N typ −102 dB** | **下がらない** |
> | ③ | 前段 PT2314 | −60 dB | — |
>
> **やってはいけない3つ:**
...
> | **ADG1407** | **9.2** | **−95.6** | **−129.1** | **−95.8** |
>
> **`ADG1407` は H3 支配で H2 と 33 dB 違う。** 総 THD だけ見ていると隠れる。
>
> **総量の段階で落とせるもの:** ②より**上**に出る `DG507B`(−70.3) と
> `ADG1407` @9.2Vrms(−95.6) は、ADC より大きいので確実に測定を汚す。**ここは確定。**
...
- **⚠ 測定床は3回間違えている。§1 の⚠と spice/README.md「判定の基準線」を必ず読む**:
  ①ランダム雑音の bin 床 -137.3dBFS（FFT長で下がる）
  ②**PCM1804 の THD+N typ -102dB（FFT長で下がらない。絶対THDを制約するのはこれ）**
  ③前段 PT2314 の -60dB
...
| 2026-09-03 深夜 | **#33 の最終2点。①総THD同士をベクトル合成しない ②実行コードの見解をドキュメントに一致させる。** ①`−102dB` は **THD+N の総量**、素子の `−115.2dB` は **H2〜H9 の RSS**。**合成できるのは同じ次数どうし**なので、13dB→+1.75dB は「仮に同一次数がそのレベルなら」の例示に格下げ。素子側は `switch_thd.py` が H2/H3 を別に返しているので `switch_compare.py` に**次数別テーブルを追加**したところ、**`ADG1407` は H3 支配（9.2Vrms で H2 −129.1 / H3 −95.8 と 33dB 差）**と判明——総THD だけ見ていた間は隠れていた。`TMUX7612` @9.2Vrms も個々の次数は −121.2/−119.7 で総量より 4〜6dB 低い。②**`switch_compare.py` が撤回済みロジック（`adc_dr=112` + `pg=10log10(2048/2)`、「−136dB より下なら汚さない」）をそのまま出力していた**のを書き換え。README だけ直すと数か月後に実行して古い誤結論が復活する状態だった。**総量で落とせるのは `DG507B` と `ADG1407`@9.2Vrms のみ確定**、残りは次数別の実測待ち |
