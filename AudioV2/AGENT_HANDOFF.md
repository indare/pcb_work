...
2. **【新規・最優先級】PCM1804 の測定能力を実測で確定する（issue #33 の到達点）**
   ...
   **源の純度は分解能にほぼ dB 対 dB で効く。** −70dB 級の源だと 0.005 dB の変化を
   ...
   現状の `Audio/measurement_fw/` は **10 バンドのオクターブ表示**で、
   `fft.py` の `power()` が `re²+im²` に潰すので**位相が捨てられている**。
   また `NORM_BITS = 15` で入力を 15bit に正規化しており、24bit ADC に対し 9bit 捨てている。

   **推奨: 生サンプルを USB で吸い上げて PC 側 numpy で解析する。**
   固定小数点の精度問題を回避でき、平均化・位相・窓関数も自由に選べる。
   `i2s_rx.py` の `start_into`/`wait` がそのまま使える。
