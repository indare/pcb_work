...
| # | 項目 | 備考 |
|---|---|---|
| A1 | **USB アイソレーションの切り分け** | 1205〜1212Hz の混入が USB ループ由来かを判定。**コストの割に情報量が大きい。構造を決める前にやる** |
| A2 | **スタック規格を決める** | 段構成・基板外形・取付穴・スペーサ・**ヘッダのピン割当**。**他は全部これに従うので最初**。ピン割当の入力データは下表（`MeasureControl` の境界信号 7 本） |
| A3 | 娘基板の粒度を決める | 2ch/4ch/5ch。**4ch×3枚（12ch容量）**が候補。接点増とのトレードオフ |
| A4 | `MeasurementADC` を何段目に置くか | ワイヤ削減とクロック隔離のトレードオフ |
| A5 | ヘッダの選定 | ±15V（電流）と音声（接触抵抗・挿抜寿命）で要求が違う |
| A6 | NetTie の位置を決め直す | `D_GND` の発生源が計測/制御基板へ移る |

**A2 の入力データ — `MeasureControl` の基板境界は 7 本（2026-09-03 確定）**
...
| # | 項目 | 備考 |
|---|---|---|
| B1 | 入力トポロジ A/B/C を決める | **数値は出揃っている**（下記の旧項目1）。A案＋1µF フィルムが有力 |
...
由来かを判定する。当たりならレイアウトでは消えない種類なので、スタック規格を
決める前に知っておきたい。次が A2「スタック規格を決める」で、他は全部これに従う。
