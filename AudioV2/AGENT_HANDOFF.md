...
| A2 | **スタック規格を決める** | 段構成・基板外形・取付穴・スペーサ・**ヘッダのピン割当**。**他は全部これに従うので最初** |
...
| B4 | 計測/制御基板の回路図（v1 から移植＋OLED・エンコーダ） | |
...
| # | 項目 | 新構成での扱い |
|---|---|---|
| D-a | ERC 53件の仕分け | 回路を作り直すので**一部は自然に消える** |
...
2026-09-03 に基板構成を作り直す方針が固まったが、**KiCad の回路図はまだ旧4枚構成**
（PowerModule / ControlPanel / AmpBank / OutputStage）。WIRING.md も旧構成のまま。
新構成は DECISIONS.md「2026-09-03 時点の基板構成案」を読むこと。
...
■ 検証
  ...
  docker/kicad-cloud-build/kicad-run.sh erc         → 53件（新構成で作り直すので一部は消える）
```
