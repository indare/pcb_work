...
| A5 | ヘッダの**部品**選定 | **2026-09-03 に要件を出した** → `DECISIONS.md`「ヘッダの部品選定 — A5」。電流容量・接触抵抗・挿抜寿命は**3つとも制約にならない**（最悪でも定格の1割 / 抵抗値でなく非線形性が論点 / D9 のおかげで抜き差しが稀）。**残っているのは ①母板側のフットプリント未指定（メスが要る） ②基板間 15mm が標準品の勘合寸法（約11mm）と合わない の2点**。`J_ANA` は金メッキ、`J_PWR_CTRL` は錫でよい（オス・メスでメッキを揃えること） |
