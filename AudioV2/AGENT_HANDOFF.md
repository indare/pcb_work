...
2. **【新規・最優先級】PCM1804 の測定能力を実測で確定する（issue #33 の到達点）**
   ...
   **当初の問い「どこまでの THD 差を再現性よく比較できるか」に直接答えるのは (a)。**
   ...
   `Audio/AmpModule` は **`DIP-8_W7.62mm_Socket` で石を差し替えられる**。
   **手持ちの石そのものを「既知の差」の梯子として使う。**

   > **⚠⚠ 回路図と実基板が違う**（2026-09-03、実物のマーキングで判明）。
   > **`AmpModule` の Rf と Rg は実基板では両方 `2001` = 2.00 kΩ で、ゲインは 2.0（非反転）。**
   > 回路図の `47k/10k`（→ ゲイン 5.7）は実装と一致しない。
   > **この差は [`AmpModule_OPAMP_REFINE.md`](../Audio/AmpModule_OPAMP_REFINE.md) §4.2 の
   > 発振判定を無効にする** —— 帰還ゼロが 3.9 MHz ではなく **31.8 MHz** になるので、
   > **`OPA1612`(−136dB) と `LME49860`(−130.5dB) が使える見込み**（`LT1364` のみ依然 NG）。
   > 詳細と表は `MeasurementADC_BRINGUP.md`「回路図と実基板が違う」。
   >
   > **教訓: 実装値を回路図から推定しない。実物のマーキングが正。**
   > 他の抵抗も一致するとは限らないので、使う前に確認すること。
