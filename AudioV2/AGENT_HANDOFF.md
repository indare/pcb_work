...
| B5 | `MeasurementADC` 次号 PCB を A2 の規格へ。**⚠ 配置がシート分割を決める**（依存の向きが逆。`DECISIONS.md`「配置がシート分割を決める」）。**Pico の置き場所に依存する部分を先に確定させないこと** | **A2 の後。先に起こすと作り直し**。**ここで「計測基板を母板に統合するか」を判断する**（2026-09-03 保留。**方向は「1枚＋娘基板のスタック」寄り**）。測定タップ `AMP_SEL_L/R` が接点を通っている点が論点。**同節の「PCB を起こすときの申し送り — アースをきちんと処理する」を必ず読むこと** |
