...
2. **【新規・最優先級】PCM1804 の測定能力を実測で確定する（issue #33 の到達点）**
   ...
   **手持ちの石そのものを「既知の差」の梯子として使う。**
   ...
   > **⚠ 訂正: `AmpModule` のゲインは `G = −47k/10k = −4.7`（反転）**で、
   > 一時「10k/10k = G=2」と書いたのは誤り（2026-09-03 に回路図で確認）。
   > `REFINE` §4.1 の「ノイズゲイン 5.7 = 1 + 47/10」とも整合する。
   > **AudioV2 の `AmpChannel`（20k/20k = G=+2）とは条件が違う。**
