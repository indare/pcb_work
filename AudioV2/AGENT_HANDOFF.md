...
| B5 | `MeasurementADC` 次号 PCB を A2 の規格へ | **A2 の後。先に起こすと作り直し**。**ここで「計測基板を母板に統合するか」を判断する**（2026-09-03 保留。`DECISIONS.md`「計測基板を母板に統合するか」）。測定タップ `AMP_SEL_L/R` が接点を通っている点が論点 |
...
| # | 項目 |
|---|---|
| C1 | 筐体設計（パネル開口: Pico USB ×1、OLED、スペアナ LCD、ポット、ジャック）。**シャーシアースを1点で落とすのもここで詰める**（A1 の結果。2点で落とすとループになる） |
