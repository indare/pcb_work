...
| A5 | ヘッダの**部品**選定 | **2026-09-03 に要件を出した** → `DECISIONS.md`「ヘッダの部品選定 — A5」。電流容量・接触抵抗・挿抜寿命は**3つとも制約にならない**（3A/接点に対し最悪1割 / 抵抗値でなく非線形性が論点 / D9 のおかげで抜き差しが稀）。**`J_ANA` は金メッキ、`J_PWR_CTRL` は錫でよい**（オス・メスでメッキを揃えること）。基板間 15mm は**ロングピン 11mm のスタッキングヘッダ（双列品は実在）＋標準 8.5mm ソケットで成立**し、D18 を変えずに済む。母板側のフットプリント（メス＝`PinSocket_2x05/2x06_P2.54mm_Vertical`）は `build_motherboard.py` に入れた。**⚠ ただし `MotherBoard.kicad_sch` は未再生成**（クラウド側に KiCad シンボルライブラリが無く、版がズレるとバイト一致が壊れるため）。**KiCad のある環境で `build_motherboard.py` を1回回すこと** |
...
| D-f | **`U202`（+9V LDO）手前の PPTC 0.1 A が未実装** | 定格は確定（`DECISIONS.md` §8）。`+15V` の T 点から `U202.1` への配線が **2.54 mm のスタブ**で 7.62 mm の素子が入らない。T 点の上流に入れると別の枝も保護してしまうので不可。**`U202` を数 mm ずらすか引き回しを変える必要があり、KiCad で見ながらやる**。30 mA の枝なので優先度は低い |
| D-g | **`U1604`（`MBC2596-01`、計測系の降圧）にヒューズを入れるか** | 未判断。`PD_12V_SW` 直付けで上流は `F1601` のみ。「各コンバータの手前に入れる」（`DECISIONS.md` §8）を徹底するならここにも要る。`MeasureControl` は手編集所有なので KiCad で直接 |
