...
> **⚠ 「測定床は PCM1804 の DR≈112 dB」は誤り**（2026-09-03 に訂正）。
> 112 dB は**広帯域**値で、スペアナは FFT（`Audio/measurement_fw/spectrum.py:19` の
> `MAX_N = 2048`）なので**離散高調波**の下限は処理利得 `10log10(N/2) = 30.1 dB` ぶん
> 下がって **−142.1 dBFS**。「`TMUX7612` の −115.2 dB は床以下だから見えない」は成り立たず、
> **床より 27 dB 上なので見える**。
> **律速するのは ADC ではなく在庫最良の DUT（`OPA1612` の −136 dB）。**
> 通常経路（PT2314 経由）は PT2314 の −60 dB が支配するので素子の差は原理的に見えないが、
> **DIRECT 経路で `OPA1612` 級の絶対 THD を測るなら 9.2 Vrms の `TMUX7612`（−115.2 dB）は
> 21 dB 足りず不適**。4〜5 Vrms（−166 dB）なら 30 dB 下で問題ない。
> つまり**振幅を膝より下に抑える設計が前提条件**になる。詳細は
> [`spice/README.md`](spice/README.md)「判定の基準線」。
...
2. **~~[issue #33](https://github.com/indare/pcb_work/issues/33) コメント2 の5点~~ → 2026-09-03 に5点とも対応済み**
   ...
   2. ✅ 判定の基準線を追加。ただし**結論は要求と逆向きになった** —— 112 dB は広帯域値で、
      FFT `N=2048` の処理利得 30.1 dB を効かせると離散高調波の下限は **−142.1 dBFS**。
      **律速は ADC ではなく在庫最良の DUT（`OPA1612` −136 dB）**（§1 の⚠を読むこと）
   3. ✅ `switch_offiso.py` に **coherent worst-case** を追加（`coherent=True` で RSS の +9.5 dB）。
      最悪の `TMUX4821` でも −139.4 dB で、**OFF漏れ非問題という結論は変わらない**
...
3. **切替素子の選択（リレー版 / アナログスイッチ版）** — §11.1a。**両案を並行検討する方針。
   ...
   歪みは**実運用 4〜5 Vrms ならどちらも十分下**なので決め手になりにくい。
   ただし **DIRECT 経路で `OPA1612`（−136 dB）級の絶対 THD を測るなら、
   `TMUX7612` を膝（±11V）より下で使うことが条件**になる（§1 の⚠）。
...
- **⚠「測定床は PCM1804 の DR≈112dB」は誤り**（2026-09-03 訂正）。112dB は広帯域値で、
  FFT N=2048（Audio/measurement_fw/spectrum.py:19）の処理利得 30.1dB を効かせると
  離散高調波の下限は **-142.1dBFS**。TMUX7612 の -115.2dB は床より 27dB 上で**見える**。
  **律速は ADC でなく在庫最良の DUT（OPA1612 -136dB）。** 通常経路は PT2314 の -60dB が
  支配するので素子差は見えないが、**DIRECT 経路で絶対THDを測るなら 9.2Vrms の TMUX7612 は
  21dB 足りない**。4〜5Vrms なら -166dB で問題なし＝**膝より下で使うのが前提**
