...
> **⚠ 「測定床」は2回間違えた場所。3つの床を混ぜないこと**（2026-09-03、issue #33）。
>
> | # | 床 | 値 | FFT長で下がるか |
> |---|---|---:|---|
> | ① | ランダム雑音の bin 床 | −137.3 dBFS | **下がる** |
> | ② | **PCM1804 自身の高調波** | **約 −102 dB** | **下がらない** |
> | ③ | 前段 PT2314 の歪み | −60 dB | — |
>
> - ①「DR 112 dB がそのまま床」は誤り（DR は広帯域の雑音指標）
> - ②「処理利得 30.1 dB を足して −142.1 dBFS が絶対 THD の床」も誤り。
>   **処理利得が効くのはランダム雑音だけで、ADC 自身の高調波（離散トーン）には効かない**。
>   加えて実使用 N は 1024（`MAX_N=2048` は確保量）、窓は Hann なので ENBW 補正も要る
> - **絶対 THD で効くのは ②。PCM1804 の `THD+N = −102 dB`（`SNR 111 dB` を引くと
>   高調波分 −102.6 dB）が実質的な床。** `OPA1612` の −136 dB は 34 dB 下で**測れない**
> - `TMUX7612` は 9.2 Vrms（−115.2 dB）でも ②より 13 dB 下なので**見えない**。
>   ②より上に出るのは `DG507B`(−70.3) と `ADG1407` @9.2Vrms(−96.1) だけ
> - **オペアンプ間の相対比較は成立する**（ADC 歪みは全DUTに共通に乗る）。この装置の主目的はそちら
>
> **⚠ ②はデータシート typ 値なので、基準信号を直接 ADC へ入れた実測で確定させること。**
> それまで ②を根拠に部品を落とさない。詳細は [`spice/README.md`](spice/README.md)「判定の基準線」。
...
2. **~~[issue #33](https://github.com/indare/pcb_work/issues/33) コメント2 の5点~~ → 2026-09-03 に5点とも対応済み**
   1. ✅ `MAX14753`/`MAX14778` の THD を**モデル値と明示**（実 `Ron(V)` 由来ではない。
      特に `MAX14778` は `Ron(V)` グラフ自体が無いので −165dB を選定根拠にしない）
   2. ✅ 判定の基準線を追加（**2度直した**。§1 の⚠が正）。**絶対 THD の床は
      PCM1804 自身の高調波 ≈ −102 dB**。112 dB も −142.1 dBFS も床ではない
   3. ✅ `switch_offiso.py` に **coherent worst-case** を追加（`coherent=True` で RSS の +9.5 dB）。
      さらに `--harmonics` で **THD 試験の実周波数**（基本波1kHz → H2=2k/H3=3k/H5=5k）を出せる。
      **20kHz 単発で「非問題」と断定しないこと** —— `TMUX4821` の −139.4 dB@20kHz は
      `OPA1612` の −136 dB と 3.4 dB しか離れていない（同相なら合成 −131.5 dB）。
      ただし 1kHz 基本波の H2 では −159.4 dB で 23 dB の余裕がある。`TMUX7612` は全域で 55 dB 余裕
   4. ✅ 入力トポロジに **C案**を追加（上記1に比較表）。副産物として
      **A案は必要Cが 0.80 µF でフィルムにでき、電解リークを回避できる**ことが分かった
   5. ✅ charge-pump spur を **「一次見積では問題なさそう、実測必須」**へ（未確定3点を明記）

   **部品選定は動いていない。** 5点は「主張の確度を正す」要求で、結論の向きが変わったのは
   2 の測定床だけ（そしてそれは `TMUX7612` を**より厳しく**見る方向）。

3. **切替素子の選択（リレー版 / アナログスイッチ版）** — §11.1a。**両案を並行検討する方針。
   基板として両方を設計してもよい**（ユーザー判断、2026-09-02）。
   歪みは**どちらも ADC の床（②≈−102 dB）より下なので、測定上は決め手にならない**（§1 の⚠）。
   `TMUX7612` は 9.2 Vrms でも②より 13 dB 下。**②より上に出るのは `DG507B` と
   `ADG1407` @9.2Vrms だけ**なので、この2つ以外は歪みで落ちない。
...
4. **ERC の仕分け（2026-09-02 に着手、60 → 53 件）** — 実害2件を発見し修正済み:
...
7. ControlPanel 側 I²C コネクタの物理実装。**現状 4P**（`I2C_SDA`/`I2C_SCL`/`3V3`/`D_GND`）だが、
...
- **⚠ 測定床は2回間違えている。3つを混ぜないこと**（詳細は §1 の表 / spice/README.md）:
  ①ランダム雑音の bin 床 -137.3dBFS（FFT長で下がる）
  ②**PCM1804 自身の高調波 約 -102dB（FFT長で下がらない。絶対THDで効くのはこれ）**
  ③前段 PT2314 の -60dB
  「DR 112dB が床」も「処理利得を足して -142.1dBFS」も**どちらも誤り**。
  処理利得はランダム雑音にしか効かず、ADC 自身の高調波は離散トーンなので下がらない。
  **OPA1612 の -136dB は②より 34dB 下＝この ADC では測れない**（相対比較は成立）。
  TMUX7612 は 9.2Vrms でも②より 13dB 下で見えない。②より上は DG507B と ADG1407@9.2Vrms だけ。
  **②は DS の typ 値なので実測で確定させること**
