...
| **B3a** | **`AmpBank` を 5ch 娘基板 ×2 へ分割** ← **いまここ** | いまは「10ch 1枚・TMUX 5個」で D19（5ch×2枚）の最終形ではない。1枚 5ch＝**TMUX 3個**（12スイッチ中10使用）×2枚へ。母板側のスロット（`J_ANA101/102`・`J_PWR101/102`）は既にある |
| B3b | リレー版娘基板 | `AmpChannel` が共通なので、切替シートだけ作る。ラッチング DPDT ×5 ＋ `ULN2803` ×1（コイル 10 本）。`+5V_COIL`/`GND_COIL` はヘッダに用意済み |
