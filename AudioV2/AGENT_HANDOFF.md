...
| A1 | **USB アイソレーションの切り分け** ← **残っている中ではこれが先** | 1205〜1212Hz の混入が USB ループ由来かを判定。**当たりならレイアウトでは消えない種類。**`B4'`（Pico 統合）の設計に効く |
...
| **B4'-1** | **UI と I2C マスタを計測基板へ** ← **いまここ** | ENC×3・LED×2・OLED・DEST ラダーを `MCP23017`(0x22) 経由で移す。**着手前に D27 の「系の `3V3` は `PICO_3V3` から。`+3V3_A` は ADC の清浄レールなので触らない」を読むこと。** `D_GND` の統合は D28 で片付いた（NetTie を足さなければループは閉じない） |
| B4'-2 | `PT2314` と `+5V` を母板へ、`ControlPanel` 廃止 | 20点＋`BP5293-50`＋パネル SW/LED |
...
| # | 項目 |
|---|---|
| C1 | 筐体設計（パネル開口: Pico USB ×1、OLED、スペアナ LCD、ポット、ジャック） |
