...
> 代わりに **スイッチ自身の H2/H3 を在庫 DUT の THD と直接比べる**（ADC 非依存）:
>
> | 部品 | Vrms | 最大次数 | 覆い隠す DUT |
> |---|---:|---:|---|
> | TMUX7612 | 4.0 | −166.6 | **なし（全4石を測れる）** |
> | TMUX7612 | 9.2 | −119.7 | LME49860 / OPA1612 |
> | ADG1407 | 4.0 | −117.9 | LME49860 / OPA1612 |
> | ADG1407 | 9.2 | −96.3 | MUSES02 / LME49860 / OPA1612 |
> | DG507B | 4.0 | −85.9 | **全4石** |
>
> - **`DG507B` は 4Vrms でも全 DUT を覆い隠す。明確に不適**
> - **`ADG1407` @9.2Vrms は高リスク**。落とす理由は「②より上」ではなく
>   **「素子自身が H3 −96dB を出すから」**
> - **⚠ `TMUX7612` @9.2Vrms も在庫最良の2石を覆い隠す。**
>   **全4石を測れるのは `TMUX7612` @4Vrms だけ** ＝ **「膝より下で使う」は
>   どの DUT まで測れるかを直接決める設計条件**
>
> **「相対比較は成立」も言い過ぎ。** 正しくは
...
4. **切替素子の選択（リレー版 / アナログスイッチ版）** — §11.1a。**両案を並行検討する方針。
   ...
   ただし**無関係でもない** —— ADC 非依存の落選判定（§1）では
   **`TMUX7612` @9.2Vrms が `LME49860`/`OPA1612` を覆い隠す**ので、
...
| 2026-09-03 深夜3 | **#33 最終。①説明文のハードコード ②落選判定の自己矛盾を解消。** ①次数別テーブルは `a.stage` に追従するようにしたのに、直後の説明文が output-only の数値をハードコードしたままだった（`--stage both` だと表と説明が食い違う）。**説明文も実測値から動的生成**するようにした。②**「①②は確定床ではない」と書いた直後に「②より上だから確実に測定を汚す」と②を確定床として部品を落としていた**——自己矛盾。とくに `ADG1407`(−96.1) と ②(−102) の差は 6dB しかなく typ 値で断定できない。**落選判定を ADC 非依存の基準へ置換**: **スイッチ自身の H2/H3 を在庫 DUT の THD と直接比べ、DUT より大きければその石は測れない**。結果、新事実が出た —— **`TMUX7612` @9.2Vrms も `LME49860`/`OPA1612` を覆い隠す**（H3 −119.7）。**在庫4石すべてを測れるのは `TMUX7612` @4Vrms だけ**で、**「膝より下で使う」はどの DUT まで測れるかを直接決める設計条件**だと判明。`DG507B` は 4Vrms でも全4石を覆い隠すので ADC を持ち出すまでもなく不適 |
