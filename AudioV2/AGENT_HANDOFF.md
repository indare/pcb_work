...
2. **【新規・最優先級】PCM1804 の測定能力を実測で確定する（issue #33 の到達点）**
   ...
   **手持ちの石そのものを「既知の差」の梯子として使う。**
   ...
   > **⚠⚠ 回路図と実装が違う**（2026-09-03、発注記録 `SUSUMU RG3216P-1002-B-T1` で確定）。
   > **`AmpModule` の Rf と Rg は両方 10.0 kΩ でゲインは 2.0（非反転）。**
   > 回路図の `R405/R408 = 47kΩ` は実装と一致しない。
   > ゲイン 2.0 は AudioV2 の `AmpChannel`（20k/20k）と同じなのは好都合。
   >
   > **⚠ `REFINE` §4.2 の発振判定は「実装の方が 1.73 倍きびしい」方向へずれる**
   > （ゼロは 3.9→6.4 MHz と上がるが、ノイズゲインが 5.7→2.0 に下がって交差が 2.85 倍上がる）。
   > **安全に使えるのは `OPA2134`(1.59×) / `NE5532`・`NJM5532`(1.16×) / `OPA2140`(1.16×) /
   > `MUSES01・02・03`(1.06×) だけ。**
   > `NJM4580` / `OPA1652` / `OPA2604` は REFINE では OK 側だったが**実装値では NG**。
   > `OPA1612` / `LME49860` / `OPA1656` / `LT1364` は依然 NG。
   > 詳細は `MeasurementADC_BRINGUP.md`「回路図と実基板が違う」。
   >
   > **教訓: 実装値を回路図から推定しない。発注記録か実測が正。**
   > この件で3回間違えた（座標→回路図値→マーキング誤読）。

   **実装値で組み直した梯子（発振しない石のみ）:**

   | 石 | THD+N | 差 | GBW | 安定余裕 |
   |---|---:|---:|---:|---:|
   | NJM5532DD / MUSES01 | ≈−94 | — | 10〜12M | 1.06〜1.16× |
   | MUSES02D | −100.0 | −6.0 | 12M | 1.06× |
   | **OPA2134PA** | **−122.0** | **−22.0** | **8M** | **1.59×（最安全）** |
   | OPA2140AIDR | −126.0 | −4.0 | 11M | 1.16× |
   | MUSE03 | −130.5 | −4.5 | 12M | 1.06× |
