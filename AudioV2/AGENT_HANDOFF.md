...
| # | 項目 | 備考 |
|---|---|---|
| **B0** | **`AmpChannel` を「出力のみ MUX」へ揃える** ← **B3 の前提** | 回路図も生成コードも**まだ「入力も出力も MUX」**（`TMUX7612` 1個/ch、4スイッチを 入力L/R・出力L/R に使用）。§11.1a の決定は「入力ブロードキャスト＋出力のみ MUX、×5」。**IC 数が 10→5 になり B1 とも直結する**。ヘッダを渡る信号は変わらないので D18 には影響しない |
| B1 | 入力トポロジ A/B/C を決める | **数値は出揃っている**（下記の旧項目1）。A案＋1µF フィルムが有力。**B0 と同時に決まる** |
