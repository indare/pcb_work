...
| A5 | ヘッダの**部品**選定 | **2026-09-03 に要件を出した** → `DECISIONS.md`「ヘッダの部品選定 — A5」。電流容量・接触抵抗・挿抜寿命は**3つとも制約にならない**（3A/接点に対し最悪1割 / 抵抗値でなく非線形性が論点 / D9 のおかげで抜き差しが稀）。**`J_ANA` は金メッキ、`J_PWR_CTRL` は錫でよい**（オス・メスでメッキを揃えること）。基板間 15mm は**ロングピン 11mm のスタッキングヘッダ（双列品は実在）＋標準 8.5mm ソケットで成立**し、D18 を変えずに済む。**残っているのは母板側のフットプリント未指定（メスが要る）だけ** |
