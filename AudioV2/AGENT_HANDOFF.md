...
| A6 | NetTie の位置を決め直す | `D_GND` の発生源が計測/制御基板へ移る |
...
| **B4'-1** | **UI と I2C マスタを計測基板へ** ← **いまここ** | ENC×3・LED×2・OLED・DEST ラダーを `MCP23017`(0x22) 経由で移す。**着手前に D27 の実装計画2点（`3V3` の出所 / `D_GND` の統合＝A6）を読むこと** |
