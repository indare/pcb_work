...
| 用途 | 推奨ジャンパ | 備考 |
|---|---|---|
| **DKMW20F-12** 一次 | **12 V** ジャンパ | PowerModule **再設計**・板上 |
| パネル **12 V LED** | 操作パネル PWR SW 後 | `PD_12V` 端子 ↔ パネル ↔ `PD_12V_SW` 戻り |
| 将来 20 V PD | 20 V | 余裕確保用。F-12 の入力上限 36 V 内 |

実装は **50224 モジュール実装位置**でも **CH224K 直付け**でも可 — いずれも **AudioV2 PowerModule 再設計**に内蔵。
