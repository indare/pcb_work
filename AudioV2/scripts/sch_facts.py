...
⚠ これは `kicad-cli` の代わりではない。ERC の正は `docker/kicad-cloud-build/kicad-run.sh erc`。
   ...
   - **部品数**: instance path を展開したユニーク参照が **373 個**で、CLAUDE.md が載せている
     `kicad-cli` netlist の期待値と一致した。**ここがずれたら再構成が壊れているサイン**
   - **ピン座標**: シンボル配置＋回転＋ミラーから出したピン先が、wire / label / junction /
     no_connect のいずれかに載る。載らなかった 12 本は `MotherBoard` の PT2314 未使用入力で、
     CIRCUIT_DESIGN.md §7 が TODO として挙げているものと一致した
