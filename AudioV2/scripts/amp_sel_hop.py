...
サブコマンド:
  ...
  drc      kicad-cli の DRC を基準リビジョンと比べる（新しい違反 0・基板ごとの未接続数が不変なら 0 で終わる）
    ...
    python3 AudioV2/scripts/amp_sel_hop.py propose --net /AMP_SEL_L --from 557.675,170.6 --to 564.875,170.6
    python3 AudioV2/scripts/amp_sel_hop.py apply --pad U311:7 --via 557.675,170.6 --path 557.675,170.6 564.875,170.6
    python3 AudioV2/scripts/amp_sel_hop.py drc --base 3cc2748
...
def _stage(pcb_bytes: bytes, dest: Path) -> Path:
    ...
    if dest.exists():
        shutil.rmtree(dest)
    (dest / "AudioV2").mkdir(parents=True)
    ...
    for e in src.iterdir():
        ...
        if e.name == "AudioV2Case.kicad_pro":
            shutil.copy2(e, dest / "AudioV2" / e.name)
        else:
            os.symlink(e, dest / "AudioV2" / e.name)
    pcb = dest / "AudioV2" / "AudioV2Case.kicad_pcb"
...
def cmd_drc(a) -> int:
    ...
    if not base_json.exists():
        blob = subprocess.run(["git", "show", f"{base_rev}:AudioV2/AudioV2Case.kicad_pcb"], cwd=ROOT,
