...
def prune(sheet: sch_import.Sheet, verbose: bool = False) -> dict[str, int]:
    ...
    2026-09-04 の実測: 母板から部品1個を外したらラベル 27 個・階層ラベル
    13 個が一緒に消えた（階層ラベルはシート界面なので親子の接続が壊れる）。
    素材の legacy/*.kicad_sch（手描き・ラベルはワイヤに乗る）では正しく動く。
    生成シートから部品を外すなら素材を編集して再生成する（CLAUDE.md）。
