...
**⚠ 2026-09-24 からルートは手編集所有**（CLAUDE.md「シートの所有権」）。`build_motherboard.py` は安全弁で止まるので、
**ここを直しても設計には届かない**。以下は、ルートがまだ生成物だった頃の記録。

**（当時）ここは「凍結した素材」であって「無効なファイル」ではなかった。**
[`../scripts/build_motherboard.py`](../scripts/build_motherboard.py) が**読んでルートに取り込んでいた**ので、
**ここを直すと設計に届いた**（2026-09-03 に PPTC 追加でここを編集し、実際に反映された）。
