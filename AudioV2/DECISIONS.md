...
| 面積 | 28 cm²（基板を **200×100 mm** へ） | 小さい |
| 付帯 | ドライバ ULN2803 ×5・40本の駆動線・`+5V` | 不要（MCP23017 ×1 のまま） |
...
| 外形 | 14 × 9 × 5 mm。20個で約 28 cm² |
| 駆動 | 2コイルラッチなので 40 本。ULN2803 ×5 ＋ GPIO 拡張。**コイル電流はパルス時のみ** |
| 費用 | 約 ¥8,900（ドライバ込、`TQ2-L2-5V` qty25 ¥406 基準） |
...
- 「面積を食う」→ **残る**。28 cm² を払う。基板を **200×100 mm** に広げる（150×100 では
  DIP-8 ソケット10個と合わせて 59% 占有になり、40本のコイル配線が通らない）

#### 実装への波及

- **`+5V` が AmpBank に必要になる**（コイル駆動）。2026-09-02 に「アナログスイッチ化で不要」
  として ControlPanel のインタフェースから外したので、**戻すか AmpBank にレギュレータを置く**かの判断が要る
- ドライバ（ULN2803 ×5）と GPIO 拡張（MCP23017 を1個から増やす、またはデコーダ）
