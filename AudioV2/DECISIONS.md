...
1. **9.2 Vrms は通常経路では到達しない。** PT2314 のクリップが 2〜2.5 Vrms なので
   ...
   **この実レベルなら `TMUX7612` は −166.5 dB** で、「9.2 Vrms で 47 dB 損する」という
   リレー選定の根拠の一つが成立しない
2. **測定床の議論は2回間違えた。3つの床を混ぜないのが結論**（2026-09-03、issue #33）。
   ...
   - ~~「測定床は DR ≈ 112 dB」~~ 誤り。DR は**広帯域の雑音**指標で歪みの床ではない
   - ~~「処理利得 30.1 dB を足して −142.1 dBFS」~~ も誤り。**処理利得が効くのは
     ランダム雑音のビン当たり密度だけで、ADC 自身の高調波（離散トーン）には効かない**。
     加えて実使用 N は 1024（`MAX_N = 2048` は確保量）、窓は Hann なので ENBW 補正も要る
   - **絶対 THD で効くのは ②。** PCM1804 の `THD+N = −102 dB`（`VIN = −0.5 dB`）から
     `SNR 111 dB` を引くと高調波分は −102.6 dB。**THD+N はほぼ高調波で占められている**
   - **`OPA1612` の −136 dB は ②より 34 dB 下で、この ADC では測れない。**
     ただし**オペアンプ間の相対比較は成立する**（ADC 歪みは全DUTに共通に乗る）。
     この装置の主目的はそちらなので、目的自体は損なわれない
   - `TMUX7612` は 9.2 Vrms（−115.2 dB）でも ②より 13 dB 下で**見えない**。
     ②より上に出るのは `DG507B`(−70.3) と `ADG1407` @9.2Vrms(−96.1) だけ

   **⚠ ②はデータシートの typ 値（条件は `VIN = −0.5 dB` の1点）。**
   基準信号を直接 ADC へ入れた実測（できればレベルスイープ）で確定させること。
   それまで ②を根拠に部品を落とさない。
...
**どちらを採るかは「面積とドライバ複雑さ」対「実装容易性と調達の堅さ」の判断**。
**歪み性能は決め手にならない** —— `TMUX7612` は 9.2 Vrms でも ADC の床（②≈−102 dB）より
13 dB 下で、測定に現れない。リレーは構造的にゼロだが、**測れない差なので実利にならない**。

ただし ②が実測で大きく良い方（−120 dB 級）へ動いた場合は再評価が要る。
そのとき `TMUX7612` は膝（±11V）より下で使う制約が付くが、リレーには付かない。
