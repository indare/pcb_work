...
1. **9.2 Vrms は通常経路では到達しない。** PT2314 のクリップが 2〜2.5 Vrms なので
   ...
   **この実レベルなら `TMUX7612` は −166.5 dB** で、「9.2 Vrms で 47 dB 損する」という
   リレー選定の根拠の一つが成立しない
2. **測定床は PCM1804 の DR ≈ 112 dB。** `TMUX7612` は 9.2 Vrms でも −115.2 dB で
   **既に測定床以下**。−140 dB 以下を追う価値は音質ではなく IC数・面積・価格に移る
...
**どちらを採るかは「面積とドライバ複雑さ」対「実装容易性と調達の堅さ」の判断**で、
歪み性能はもはや決め手にならない（実運用 4〜5 Vrms ではどちらも測定床の遥か下）。
