...
| PD 給電 | **確定** | **モジュール差し替え可**（15 V PD / 他規格）。AudioV2 PowerModule で J 端子＋DC-DC を設計 |
...
4. PT2314 外部 C/R（トーン）
5. **±12 V 化に伴うレベル**（Amp ヘッドルーム、HP −20 dB パッド要否）
...
| PD 入力 | **給電モジュールは差し替え可能**（端子台 or コネクタで PD 規格を選べる） |
| パネル 12 V LED | アナログ ±12 V **とは別**。PD 後段 or 専用レギュレータから **+12 V 単極**を取る（要設計） |
...
| HP **−20 dB パッド**が過剰になり得る | 回路起こし時に **要否・値を再計算**（DECISIONS §2 の固定パッド） |
...
```text
[PD 給電モジュール（差し替え可）] ──► DKMW20F-12 ──► ±12V / A_GND
                                              ├──► Amp / Buffer / PGA2310 / 計測（±12V_A）
                                              └──► 操作 Pico 用 +5V 等（別途）
