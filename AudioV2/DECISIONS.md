...
2. **測定床の議論は3回間違えた。3つの床を混ぜないのが結論**（2026-09-03、issue #33）。
   ...
   **「床より下だから見えない」も誤り。** 同じ次数の成分は複素ベクトル和で残り、
   ...
   | マージン | 誤差 | 該当 |
   |---:|---:|---|
   | 13 dB | **+1.75 dB** | **`TMUX7612` @9.2Vrms** |
   | 34 dB | +0.17 dB | `OPA1612`(−136dB) が ADC に乗る場合 |
   | 64 dB | +0.005 dB | `TMUX7612` @4〜5Vrms |

   - ②より**上**に出るのは `DG507B`(−70.3) と `ADG1407` @9.2Vrms(−96.1) だけ
   - **`TMUX7612` @9.2Vrms は「ADC より下だから無条件 OK」ではない**（マージン 13 dB）。
     4〜5 Vrms なら 64 dB マージンで問題なし ＝ **膝より下で使うのが前提**
