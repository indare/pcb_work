...
1. **9.2 Vrms は通常経路では到達しない。** PT2314 のクリップが 2〜2.5 Vrms なので
   ...
   **この実レベルなら `TMUX7612` は −166.5 dB** で、「9.2 Vrms で 47 dB 損する」という
   リレー選定の根拠の一つが成立しない
2. ~~**測定床は PCM1804 の DR ≈ 112 dB。**~~ **これは誤りだった**（2026-09-03 訂正）。
   112 dB は**広帯域**値で、スペアナは FFT（`Audio/measurement_fw/spectrum.py:19` の
   `MAX_N = 2048`）なので**離散高調波**に対する下限は処理利得 `10log10(N/2) = 30.1 dB`
   ぶん下がって **−142.1 dBFS**。`TMUX7612` の −115.2 dB は床より **27 dB 上で見える**。
   **律速するのは ADC ではなく在庫で最良の DUT（`OPA1612` の −136 dB）。**
   - 通常経路（PT2314 経由、4〜5 Vrms）: PT2314 の **−60 dB** が支配。素子の差は原理的に見えない
   - **DIRECT 経路（PT2314 迂回）で `OPA1612` 級の絶対 THD を測るなら、切替素子は −136 dB より
     十分下が要る。9.2 Vrms の `TMUX7612`（−115.2 dB）は 21 dB 足りず不適**。
     4〜5 Vrms（−166 dB）なら 30 dB 下で問題ない
   - つまり **`TMUX7612` を採るなら「膝（±11V）より下で使う」ことが設計の前提条件**になる
...
**どちらを採るかは「面積とドライバ複雑さ」対「実装容易性と調達の堅さ」の判断**が主。
歪み性能は**実運用 4〜5 Vrms ならどちらも十分下**なので決め手になりにくいが、
**完全に無関係にはならない** —— DIRECT 経路で `OPA1612`（−136 dB）級の絶対 THD を測る用途では、
`TMUX7612` は膝より下で使う制約が付く。リレーにはその制約が無い（接触抵抗は電圧非依存）。
