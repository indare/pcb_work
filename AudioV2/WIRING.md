...
> **⚠ 2026-09-03: 基板構成を作り直す方針が固まった。この文書はまだ旧構成（4枚）のまま。**
> 新構成は **母板（アナログ）＋ `AmpBank` 娘基板 ×N（番地付き）＋ 計測/制御基板** の3種で、
> 基板間はワイヤではなく**ヘッダのスタック**。`ControlPanel` は無くなり、`OutputStage` は
> 母板へ統合、`PT2314` も母板へ移る。**PC への USB は 2本→1本。**
> 詳細と理由は [`DECISIONS.md`](DECISIONS.md)「2026-09-03 時点の基板構成案」。
> **KiCad の回路図も未反映なので、着手前に必ず DECISIONS を読むこと。**
>
...
- **NetTie 位置: ~~ControlPanel~~ → 計測/制御基板**（`D_GND` の発生源＝**計測 Pico** の直近）。
  ...
  **`MeasureControl` の `NT1601`/`NT1602` が既にその1点なので、他に追加してはいけない**
